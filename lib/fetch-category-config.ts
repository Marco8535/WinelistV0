    ...
    headers.forEach((header, index) => {
      ...
      if (key) {
        if (key === "displayOrder") {
          ...
          entry[key] = isNaN(num) ? Number.POSITIVE_INFINITY : num // Si no es número, va al final
        } else {
...
export async function fetchCategoryConfig(): Promise<CategoryConfig[]> {
  try {
    const csvUrl =
      "https://docs.google.com/spreadsheets/d/e/2PACX-1vSYJ_osaRfpE560QPeFdseyGCdyo0PQ10y0MutQYBaJXzk4b0oIs5twb1BliBePIANRiv0Qat_iftYF/pub?gid=1602638745&single=true&output=csv"
